/*
    <DLC : DATA CONTRIL LANGUAGE> 데이터 제어 언어
    계정에게 시스템 권한 또는 객체접근할 수 있는  권한 부여(GRANT) 하거나 회수(REVOKE)하는 구문
    
    >> 시스템 권한 : DB에 접근하는 권한, 객체들을 생성 할 수 있는 권한
        * 종류 
...
GRANT CREATE SESSION TO SAMPLE;
    -- 접속하여 테이블을 만들면 오류. 테이블을 만들수 있는 권한이 없음
...
-- 5. SAMPLE계정에게 tjoeun계정에 EMPLOYEE 테이블을 SELECT 할 수 있는 권한 부여
GRANT SELECT ON TJOEUN.EMPLOYEE TO SAMPLE;

-- 6. SAMPLE계정에게 tjoeun계정에 EMPLOYEE 테이블을 INSERT 할 수 있는 권한 부여
GRANT INSERT ON TJOEUN.EMPLOYEE TO SAMPLE;

-- *권한 회수
-- REVOKE 회수할 권한 ON 특정 객체 FROM 계정명;
