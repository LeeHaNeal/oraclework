/*
    * DML(DATA MANIPLATION LANGUAGE) : 데이터 조작언어
    : 테이블에 값을 검색(SELECT), 삽입(INSERT), 수정(UPDATE), 삭제(DELETE)한느 구문
*/
--==============================================================================================
/*
    1. INSERT
        ...
        1) INSERT INTO 테이블명 VALUES(값1,값2,값3....)
            ...
            값을 많이 넣을 경우 -> too many value오류
*/

-- EMPLOYEE 테이블에 한 행 추가
INSERT INTO EMPLOYEE VALUES(223,'더조은','980924-1234567','tjoeun@or.kr','01012345678','D2','J1',3500000,0.2,200,
...
/*
    ...
    그래도 한 행 단위로 추가됨.
    -> 선택이 안된 컬럼은 NULL이 들어감. DEFALUT가 지정되어 있으면 NULL이 아닌 DEFAULT값이 들어감
...
WHEN HIRE_DATE >= '2000/01/01' THEN 
    ...
    FROM EMPLOYEE;
-- 급여가 3백만원보다 많이 받는사람을 넣을 테이블, 적게받는사람을 넣을 테이블
...
WHEN SALARY >= 3000000 THEN
    INTO EMP_MAX VALUES(EMP_ID,EMP_NAME,SALARY,BONUS)
WHEN SALARY <3000000 THEN
    INTO EMP_MIN VALUES(EMP_ID,EMP_NAME,SALARY,BONUS)
...
UPDATE EMP_SALARY
SET SALARY = 
(SELECT SALARY FROM EMP_SALARY WHERE EMP_ID = '206') , 
...
-- 조정연,유하보,강정보 직원들의 급여와 보너스를 문정보사원과 동일하게 인상 
UPDATE EMP_SALARY
SET (SALARY,BONUS) =  (SELECT SALARY , BONUS 
                        FROM EMP_SALARY 
                        WHERE EMP_NAME = '문정보')
