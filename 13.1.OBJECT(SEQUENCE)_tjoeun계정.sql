...
/*
    * 시퀀스 객체 생성
    ...
    [START WITH 시작숫자]   -> 처음 발생시킬 시작값 짖어(기본값 1)
    [INCREMENT BY 숫자]       -> 몇씩 증가 시킬지
    [MAXVALUE 숫자]       -> 최대값 지정(기본값 엄청큼)
    [MINVALUE 숫자]       -> 최소값 지정(기본값 1)
    [CTCLE | NO CYCLE]      -> 값 순환 여부 지정(기본값 NOCYCLE)
    [NOCAHCE | CACHE 바이트 크기)    -> 캐시메모리 할당(기본값 CACHE 20)
    > 캐시메모리 : 시퀀스의 값을 미리 생성하여 저장해 두는곳
                    매번 호출할 때마다 새롭게 번호를 생성하는게 아니라
                    캐시 메모리에 미리 만들어 둔 값을 가져다 씀(속도가 빨라짐)
                    접속해제 -> 캐시메모리에 미리 만들어 둔 번호는 다 날라감
