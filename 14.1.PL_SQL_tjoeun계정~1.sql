/*
    * PL / SQL
        ...
        SQL 문장 내에서 변수 저의, 조건 처리(IF). 반복문(LOOP,FOR,WHILE)등을 지원
        
     * PL / SQL 구조
     - [선언부 (DECLARE SECTION)] : DECLARE로 시작 ,변수나 상수정의 및 초기화 하는 부분
    - 실행부 (EXECUTABLE SECTION) : BEGIN으로 시작, SQL문 또는 제어문(조건문,반복문)
...
-- 출력문 : HELLO ORACLE
BEGIN
-- SYSTEM.OUT.PRINLN("HELLO ORACLE"); <- 자바
    DBMS_OUTPUT.PUT_LINE('HELLO OREACLE');
END;
/
/*
    1. DECLARE 선언부
...
-- 1.2) 래퍼런스 스타일 변수 선언 및 초기화 (테이블의 컬럼의 데이터타입을 참조하여 그타입으로 지정)
...
DECLARE
    E EMPLOYEE%ROWTYPE;
BEGIN
    SELECT *  -- 무조건 *을 사용 EMP_ID,EMP_NAME사용하면 오류 발생
    INTO E
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
...
-- 단, 보너스를 받지않는 사원은 보너스율 출력전 '보너스를 받지 않는 사원입니다' 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SAL EMPLOYEE.SALARY%TYPE;
...
-- 2) IF조건식 THEN 실행내용 ELSE 실행내용 END IF; (IF-ELSE문)
...
/*
    ...
    실행 : 사용자가 입력한 사번에 사번 , 이름 , 부서명 , 근무국가코드를 변수에 대입
            단) NCODE 값이 KO일때 => TEAM 변수에 '국내팀'
...
BEGIN
    ...
    FROM EMPLOYEE 
    JOIN DEPARTMENT  ON (DEPT_CODE = DEPT_ID)
    JOIN LOCATION  ON (LOCATION_ID = LOCATION_ID)  
    WHERE EMP_ID = &사번;
