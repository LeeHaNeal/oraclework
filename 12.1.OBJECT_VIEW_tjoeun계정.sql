...
-- INSERT
INSERT INTO VW_SAL VALUES(400, '아무개', '901023-2738465', 'J1', 3000000, 36000000); -- 오류
...
/*
    * VIEW 옵션
    ...
    - OR REPLACE : 기존에 동일한 이름의 뷰가 존재하면 덮어쓰기, 존재하지않으면 새로 생성
    - FORCE | NOFORCE
    > FORCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰를 생성할 수 있다.
    > NOFORCE : 서브쿼리에 기술된 테이블이 반드시 존재 해야만 뷰를 생성할 수 있다.(기본값)
    - WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML가능 하도록함
    - WITH READ ONLY : 뷰를 조회만 가능(SELECT 를 제외한 DML 불가)
*/
